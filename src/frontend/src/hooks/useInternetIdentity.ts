...
export function InternetIdentityProvider({
  ...
  const handleLoginSuccess = useCallback(
    (client?: AuthClient) => {
      const source = client ?? authClient;
      const latestIdentity = source?.getIdentity();
      if (!latestIdentity) {
  ...
  const login = useCallback(() => {
    ...
    // If the user already has a valid, non-expired session, reuse it
    // instead of opening the II popup or throwing an error.
    if (
